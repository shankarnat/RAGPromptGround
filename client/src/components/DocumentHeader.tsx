    ...
    <header className="h-16 border-b border-gray-200 flex items-center justify-between px-6 bg-white">
      ...
      <div className="flex items-center space-x-4">
        <div className="inline-flex rounded-md shadow-sm" role="group">
          ...
          <button 
            type="button" 
            className={`py-2 px-4 text-sm font-medium border-t border-b border-gray-200 focus:z-10 focus:ring-2 focus:ring-primary-500 ${
              processingMode === "idp" 
                ? "text-primary-600 bg-primary-50" 
                : "text-gray-900 bg-white hover:bg-gray-100"
            }`}
            onClick={() => onProcessingModeChange("idp")}
          >
            IDP
          </button>
          <button 
            type="button" 
            className={`py-2 px-4 text-sm font-medium rounded-r-md border border-gray-200 focus:z-10 focus:ring-2 focus:ring-primary-500 ${
              processingMode === "kg" 
                ? "text-primary-600 bg-primary-50" 
                : "text-gray-900 bg-white hover:bg-gray-100"
            }`}
            onClick={() => onProcessingModeChange("kg")}
          >
            KG
          </button>
        </div>
